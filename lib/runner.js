...
var path = require('path'),
    ...
    CoreConfiguration = require('./config/core_test_configuration'),
    NativeConfiguration = require('./config/native_test_configuration'),
...
var findMongo = function(packagePath) {
  if (fs.existsSync(f('%s/package.json', packagePath))) {
    ...
    if (obj.name && (obj.name === 'mongodb-core' || obj.name === 'node-mongodb-native')) {
      return {
        path: packagePath,
        package: obj.name
      };
...
console.log(mongoPackage.package);
...
try {
  ...
  } else if (mongoPackage.package === 'node-mongodb-native') {
    dbConfig = new NativeConfiguration(config);
  }
} catch (err) {
  throw new Error('The test runner must be a dependency of mongodb-core');
}
